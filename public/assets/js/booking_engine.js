...
(function () {
    ...
    function renderSelectionModalBody() {
        ...
        const renderItem = (s) => `
            <div class="service-list-item d-flex align-items-center p-2 p-sm-3 mb-2 rounded-3 w-100" 
                 style="background: #1f2736; border: 1px solid rgba(255,255,255,0.08); cursor: pointer; transition: all 0.2s ease; overflow: hidden; box-sizing: border-box;"
                ...
                <div class="flex-grow-1 min-w-0 me-2" style="overflow: hidden; word-break: break-word;">
                    <h6 class="text-white fw-bold mb-1" style="font-size: 0.88rem; line-height: 1.35; white-space: normal; word-break: break-word; overflow-wrap: break-word;">${s.name}</h6>
                    <span class="text-white-50 small d-block" style="font-size: 0.75rem;">${BOOKING_DATA.categories[s.cat] || 'Transport'}</span>
                </div>
                <i class="fas fa-chevron-right text-white-50 ms-auto flex-shrink-0" style="font-size: 0.85rem;"></i>
            </div>
        `;
        ...
        if (trendingServices.length > 0) {
            listHtml += `
                <div class="mb-4">
                    <h6 class="fw-bold mb-3 d-flex align-items-center gap-2" style="color: #da9a28; font-size: 1.05rem;">
        ...
        if (jeddahServices.length > 0 && (state.selectedCity === 'all' || state.selectedCity === 'Jeddah')) {
            listHtml += `
                <div class="mb-4">
                    <h6 class="fw-bold mb-3" style="color: #da9a28; font-size: 1.05rem;">Jeddah</h6>
        ...
        if (makkahServices.length > 0 && (state.selectedCity === 'all' || state.selectedCity === 'Makkah')) {
            listHtml += `
                <div class="mb-4">
                    <h6 class="fw-bold mb-3" style="color: #da9a28; font-size: 1.05rem;">Makkah</h6>
        ...
        if (madinahServices.length > 0 && (state.selectedCity === 'all' || state.selectedCity === 'Madinah')) {
            listHtml += `
                <div class="mb-4">
                    <h6 class="fw-bold mb-3" style="color: #da9a28; font-size: 1.05rem;">Madinah</h6>
        ...
        modalBody.innerHTML = `
            ...
            <div class="row g-3 mb-4">
                <div class="col-md-6">
                    ...
                    <select class="form-select text-white" id="modal-city-filter" style="background: #252e3f; border: 1px solid rgba(255,255,255,0.15); border-radius: 8px; font-size: 0.95rem; padding: 10px 14px;">
                        ${cityOptions}
                    </select>
                </div>
            </div>
        ...
        if (detailsModalBody) {
                ...
                vehicleHtml += `
                    <div class="col-6 col-md-4 col-lg-3 mb-2">
                        <div class="vehicle-option-card p-2 rounded text-center ${isSelected ? 'active-vehicle' : ''}" 
                             style="background: ${isSelected ? 'rgba(218, 154, 40, 0.2)' : 'rgba(255,255,255,0.03)'}; border: 1.5px solid ${isSelected ? '#da9a28' : 'rgba(255,255,255,0.1)'}; cursor: pointer; transition: all 0.2s ease;"
                             data-vehicle-id="${v.id}">
                            <div class="fw-bold text-white small mb-1" style="font-size: 0.88rem;">${v.name}</div>
                            <div class="d-flex justify-content-center gap-2 text-white-50 mb-1" style="font-size: 0.75rem;">
                                <span><i class="fas fa-user-friends text-warning"></i> ${v.pax}</span>
                                <span><i class="fas fa-suitcase text-info"></i> ${v.luggage}</span>
                            </div>
                            <div class="fw-bold" style="color: #da9a28; font-size: 0.95rem;">${price} SAR</div>
                        </div>
                    </div>
                `;
            });
            ...
            if (service.pickup !== false) {
                fieldsHtml += `
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control text-white" id="details-pickup-loc" list="locations-list" placeholder="e.g. Fairmont Makkah / Jeddah Airport" style="background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.2); padding: 10px;">
                    </div>
                `;
            } else {
                fieldsHtml += `
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control text-white" id="details-pickup-loc" value="Airport Arrival Terminal" style="background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.2); padding: 10px;">
                    </div>
                `;
            }
            ...
            if (service.dropoff !== false) {
                fieldsHtml += `
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control text-white" id="details-dropoff-loc" list="locations-list" placeholder="e.g. Pullman Zamzam Madinah" style="background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.2); padding: 10px;">
                    </div>
                `;
            }
            ...
            if (service.flight) {
                fieldsHtml += `
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control text-white" id="details-flight-num" placeholder="e.g. SV 1234 / PK 701" style="background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.2); padding: 10px;">
                    </div>
                    <div class="col-md-6 mb-3">
                        <label class="form-label text-white small fw-bold"><i class="fas fa-clock text-info me-1"></i> Flight Date & Time</label>
                        <input type="datetime-local" class="form-control text-white" id="details-flight-datetime" style="background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.2); color-scheme: dark; padding: 10px;">
                    </div>
                `;
            }
            ...
            fieldsHtml += `
                <div class="col-md-6 mb-3">
                    ...
                    <input type="date" class="form-control text-white" id="details-pickup-date" value="${defaultDate}" style="background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.2); color-scheme: dark; padding: 10px;">
                </div>
                <div class="col-md-6 mb-3">
                    <label class="form-label text-white small fw-bold"><i class="fas fa-clock text-warning me-1"></i> Pickup Time</label>
                    <input type="time" class="form-control text-white" id="details-pickup-time" value="${defaultTime}" style="background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.2); color-scheme: dark; padding: 10px;">
                </div>
            `;
            ...
            fieldsHtml += `
                <div class="col-md-6 mb-3">
                    ...
                    <input type="number" class="form-control text-white" id="details-pax" value="2" min="1" max="50" style="background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.2); padding: 10px;">
                </div>
                <div class="col-md-6 mb-3">
                    <label class="form-label text-white small fw-bold"><i class="fas fa-user-check text-success me-1"></i> Lead Passenger Name</label>
                    <input type="text" class="form-control text-white" id="details-customer-name" placeholder="Full Name" style="background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.2); padding: 10px;">
                </div>
            `;
            ...
            if (service.offset != null) {
                ...
                offsetBannerHtml = `
                    <div class="col-12 mb-3">
                        <div class="p-2 rounded d-flex align-items-center gap-2 small" style="background: rgba(40, 167, 69, 0.15); border: 1px solid rgba(40, 167, 69, 0.4); color: #28a745;">
            ...
            detailsModalBody.innerHTML = `
                ...
                <div class="row mb-4 align-items-center">
                    ...
                    <div class="col-md-8">
                        <span class="badge mb-2" style="background: rgba(218, 154, 40, 0.15); color: #da9a28; border: 1px solid rgba(218, 154, 40, 0.3); font-size: 0.8rem;">
            ...
            detailsModalBody.querySelectorAll('.vehicle-option-card').forEach(card => {
                card.addEventListener('click', () => {
                    ...
                    if (vehicleObj) {
                        ...
                        card.style.background = 'rgba(218, 154, 40, 0.2)';
                        card.style.borderColor = '#da9a28';
    ...
    function sendWhatsAppBooking() {
        ...
        let text = `*NEW UMRAH TRANSPORT BOOKING*\n`;
        text += `*AL HARMAIN UMRAH TRANSPORT*\n`;
        text += `----------------------------------------\n`;
        text += `📌 *Service:* ${service.name}\n`;
        text += `🚘 *Vehicle:* ${vehicle.name} (${vehicle.pax})\n`;
        text += `💰 *Total Price:* ${state.calculatedPrice} SAR\n`;
        text += `📅 *Date & Time:* ${pickupDate} ${pickupTime}\n`;
        text += `📍 *Pickup Location:* ${pickupLoc}\n`;
        if (service.dropoff !== false) {
            text += `🏁 *Drop-off Location:* ${dropoffLoc}\n`;
        }
        if (service.flight && flightNum !== 'N/A') {
            text += `✈️ *Flight Details:* ${flightNum}\n`;
        }
        text += `👥 *Passengers:* ${pax} Pax\n`;
        text += `👤 *Customer Name:* ${name}\n`;
        text += `----------------------------------------\n`;
        text += `Please confirm my booking and driver dispatch!`;

        const encodedText = encodeURIComponent(text);
        window.open(`https://wa.me/966565476113?text=${encodedText}`, '_blank');
    }
    ...
    function renderCartSidebarItems() {
        ...
        state.cart.forEach((item, index) => {
            ...
            html += `
                <div class="cart-item p-3 mb-2 rounded" style="background: rgba(255,255,255,0.05); border: 1px solid rgba(218,154,40,0.2);">
                    <div class="d-flex justify-content-between align-items-start mb-1">
                        <h6 class="text-white fw-bold mb-0 small">${item.serviceName}</h6>
                        <button class="btn btn-link text-danger p-0 ms-2 remove-cart-item-btn" data-index="${index}" style="font-size: 0.9rem;">&times;</button>
                    </div>
                    <div class="text-warning small fw-bold">${item.vehicleName} - ${item.price} SAR</div>
                    <div class="text-white-50 small" style="font-size: 0.75rem;">📅 ${item.date} ${item.time} | 📍 ${item.pickup}</div>
                </div>
            `;
        });
...
document.addEventListener('click', function(e) {
    ...
    if (btn && btn.tagName === 'BUTTON') {
        e.preventDefault();
        window.open('https://wa.me/966565476113?text=Hello!%20I%20want%20to%20configure%20and%20book%20this%20transport%20service', '_blank');
    }
});