...
export default function PageScripts({ scripts }: { scripts: PageScript[] }) {
  useEffect(() => {
    ...
    (async () => {
      // the site's one WhatsApp helper (lib/whatsapp.mjs), before any script that uses it
      if (POLISH) await load('/polish/wa.js');
      for (const s of scripts) {
        if (cancelled) return;
        if (DEAD.includes(s.src)) continue;
        let src = VENDOR_MAP[s.src] ?? s.src;
        // the site's own scripts inject colours: use their palette-rewritten copies
        if (/^\/(assets\/js|page-js)\//.test(src)) src = polished(src);
